...
# Then the second modification is to remove the array version of the container files and volumes while preserving shorthand string forms.
	jq '. as $$a | ."$$defs".container.properties.files |= $$a."$$defs".container.properties.files.oneOf[1] | ."$$defs".container.properties.files.additionalProperties = {"oneOf":[{"$$ref":"#/$$defs/containerFile"},{"$$ref":"#/$$defs/containerFileShort"}]} | ."$$defs".container.properties.volumes |= $$a."$$defs".container.properties.volumes.oneOf[1] | ."$$defs".container.properties.volumes.additionalProperties = {"oneOf":[{"$$ref":"#/$$defs/containerVolume"},{"$$ref":"#/$$defs/containerVolumeShort"}]} | del(."$$defs".containerFile.properties.target) | del(."$$defs".containerVolume.properties.target)' schema/files/score-v1b1.json > schema/files/score-v1b1.json.for-validation
